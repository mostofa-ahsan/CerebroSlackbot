    ...
    try:
        ...
        cursor.execute(f"""
            SELECT file_name, content, 
                1 - (embedding <-> %s::VECTOR) / (SELECT MAX(embedding <-> %s::VECTOR) FROM {table_name}) AS similarity_score
            FROM {table_name}
...
def relevance_search(sentence, table_name, vector_dim=768, top_k=10):
    ...
    try:
        ...
        cursor.execute(f"""
            SELECT file_name, content, 
                (1 - (embedding <-> %s::VECTOR)) / (SELECT MAX(1 - (embedding <-> %s::VECTOR)) FROM {table_name}) AS relevance_score
            FROM {table_name}
